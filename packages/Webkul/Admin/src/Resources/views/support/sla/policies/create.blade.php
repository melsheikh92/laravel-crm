<x-admin::layouts>
    <x-slot:title>
        @lang('admin::app.support.sla.create.title')
        </x-slot>

        <x-admin::form :action="route('admin.support.sla.policies.store')">
            <div class="flex flex-col gap-4">
                <!-- Header -->
                <div
                    class="flex items-center justify-between rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300">
                    <div class="flex flex-col gap-2">
                        <x-admin::breadcrumbs name="support.sla.policies.create" />
                    ...
                    <div class="flex flex-1 flex-col gap-4">
                        <div
                            ...
                            <x-admin::form.control-group class="mb-4">
                                ...
                                <x-admin::form.control-group.control type="text" name="name" value="{{ old('name') }}"
                                    rules="required" placeholder="Type policy name" />
                                            ...
                                            <tr class="border-b bg-white dark:border-gray-700 dark:bg-gray-800">
                                                ...
                                                <td class="px-6 py-4">
                                                    <input type="number" name="rules[{{ $index }}][first_response_time]"
                                                        value="{{ old("rules.$index.first_response_time", 60) }}"
                                                ...
                                                <td class="px-6 py-4">
                                                    <input type="number" name="rules[{{ $index }}][resolution_time]"
                                                        value="{{ old("rules.$index.resolution_time", 240) }}"
                                                        class="w-full rounded border border-gray-200 px-2.5 py-2 text-sm font-normal text-gray-800 transition-all hover:border-gray-400 focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300 dark:hover:border-gray-400 dark:focus:border-gray-400"
                                                        min="1" required>
                                                    <x-admin::form.control-group.error
                        ...
                        <div class="flex items-center justify-end gap-x-2.5 mt-4">
                            <button type="submit" class="primary-button">
                                @lang('admin::app.support.sla.create.save-btn')
                            </button>
                        </div>
                    </div>
